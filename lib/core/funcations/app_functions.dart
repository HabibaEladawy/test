...
class AppFunctions {
  ...
  static String formatReviewDate(DateTime date) {
    ...
    if (difference.inDays == 0) {
      return tr(AppTexts.reviewsJustNow);
    } else if (difference.inDays == 1) {
      return tr(AppTexts.reviewsYesterday);
    } else if (difference.inDays < 7) {
      return tr(
        AppTexts.reviewsDaysAgo,
        namedArgs: {'days': difference.inDays.toString()},
      );
    } else if (difference.inDays < 30) {
      final weeks = (difference.inDays / 7).floor();
      if (weeks == 1) {
        return tr(AppTexts.reviewsWeeksAgo, namedArgs: {'weeks': '1'});
      } else if (weeks == 2) {
        return tr(AppTexts.reviewsTwoWeeks);
      } else {
        return tr(
          AppTexts.reviewsWeeksAgoPlural,
